...
export default function Card({ item }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setEmojiList((prevState) => [...prevState, ...emojiListDataResult.results]);
    })();
  }, []);
  ...
  return (
    <div onClick={handleRefresh}>
      <Link to={`/post/${id}`}>
        <div style={isImg} className={`${styles.card} ${backgroundImageURL ? styles["bg_isImg"] : styles[backgroundColor]}`}>
          <div className={styles.card_inner}>
            <div className={styles.card_data}>
              <p className={`${backgroundImageURL ? styles["white_recipient"] : styles["recipient"]}`}>To. {name}</p>
              <ul className={styles.img_box}>
                {messageList.map((data, index) =>
                  index < 3 ? (
                    <li key={data.id} style={{ position: "relative", right: `${index * 10}px` }}>
                      <img className={styles.profile_image} src={data.profileImageURL}></img>
                    </li>
                  ) : index === 3 ? (
                    <li key={data.id} style={{ position: "relative", right: `${index * 10}px` }}>
                      <div className={styles.remainder}>
                        <span className={styles.remainder_text}>+{item.messageCount - 3}</span>
                      </div>
                    </li>
                  ) : undefined
                )}
              </ul>
              <p className={`${backgroundImageURL ? styles["white_alert"] : styles["alert"]}`}>
                <span className={`${backgroundImageURL ? styles["white_count_alert"] : styles["count_alert"]}`}>{item.messageCount}</span>
                명이 작성했어요!
              </p>
            </div>
